...
template<typename F>
class rc_ai_dumb_player_inh : public player<F>{
private:
    std::unique_ptr<my_playfield> myPlayfield;
    int myPlayer_No;
    ...
    int playerTurn(const std::unique_ptr<my_playfield> &state){
        int p1 = 0, p2 = 0;
    ...
    std::unique_ptr<std::vector<int>> actions(const std::unique_ptr<my_playfield> &state){
    ...
    void makeMove(const std::unique_ptr<my_playfield> &state, int action){
        int result = -1;
    ...
    void undoMove(const std::unique_ptr<my_playfield> &state, int action){
        int result = -1;
...
public:
    rc_ai_dumb_player_inh(){
        myPlayer_No = -1;
        std::unique_ptr<my_playfield> f(new my_playfield());
        myPlayfield = move(f);
    }

    int play(const F &field){
        // copys field
        std::unique_ptr<my_playfield> f(new my_playfield(field));
        myPlayfield = move(f);
