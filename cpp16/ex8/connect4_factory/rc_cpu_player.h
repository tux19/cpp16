...
private:
     ...
     */
    bool try_move(field_array &state, int col, int player) {
        make_move(state,col,player);
        bool can_win = check_win(state,player);
        undo_move(state,col,player);
    ...
    std::vector<int> actions(const field_array &state){
        ...
        int offset = 0;
        for( int i = 0; i < playfield::width; i++){
            if(stoneat(state,mid + offset*dir, 0) == playfield::none)
    ...
    int try_line(const field_array &state, int player, int col, int row, int colOffset, int rowOffset){
        ...
        for(int count = 0; count < 4; ++count){
            ...
            if(stoneat(state, col, row) == (player== playfield::player1 ? playfield::player2 : playfield::player1) )
                return 0;
    ...
    void undo_move(field_array &state, int action){
        int player = (player_turn(state)== playfield::player1 ? playfield::player2: playfield::player1);

        int result = -1;
        for(int r = 0; r < playfield::height && result != 0; ++r) {
    ...
    void make_move(field_array &state, int action, int player){
        int result = -1;
        for(int r = playfield::height - 1; r >= 0 && result != 0; --r) {
            if(stoneat(state, action, r) == playfield::none) {
                setstoneat(state, action,r, player);
    ...
    void undo_move(field_array &state, int action, int player){
...
public:
    ...
    int play(const playfield &field){
        ...
        for(int c : possibleCol){
            // check if I can win
            if(try_move(_field, c, my_player_no)){
                selectedCol = c;
            }

            // check if other player can win
            if(try_move(_field, c, my_player_no == playfield::player1 ? playfield::player2 : playfield::player1)) {
                selectedCol = c;
            }
        }
